...
$id      = required_param('id', PARAM_INT); // ID in bookingform_notice.
...
$notice = null;
if ($id > 0) {
    $notice = $DB->get_record('bookingform_notice', array('id' => $id));
}

$PAGE->set_url('/mod/bookingform/sitenotice.php', array('id' => $id, 'd' => $d, 'confirm' => $confirm));
...
$title = get_string('addnewnotice', 'bookingform');
if ($notice != null) {
    $title = $notice->name;
}
...
if (!empty($d)) {
    ...
    if (!$confirm) {
        ...
        echo $OUTPUT->confirm(get_string('noticedeleteconfirm', 'bookingform', $info),
        ...
        $DB->delete_records('bookingform_notice', array('id' => $id));
        $DB->delete_records('bookingform_notice_data', array('noticeid' => $id));
...
$mform = new mod_bookingform_sitenotice_form(null, compact('id', 'customfields'));
...
if ($fromform = $mform->get_data()) { // Form submitted.
    ...
    if ($notice != null) {
        $todb->id = $notice->id;
        $DB->update_record('bookingform_notice', $todb);
    } else {
        $notice = new stdClass();
        $notice->id = $DB->insert_record('bookingform_notice', $todb);
    }
    ...
    foreach ($customfields as $field) {
        ...
        bookingform_save_customfield_value($field->id, $fromform->$fieldname, $notice->id, 'notice');
    }
    $transaction->allow_commit();
    redirect($returnurl);
    ...
    foreach ($customfields as $field) {
        $fieldname = "custom_$field->shortname";
        $toform->$fieldname = bookingform_get_customfield_value($field, $notice->id, 'notice');
    }

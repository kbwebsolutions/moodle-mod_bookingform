...
class backup_bookingform_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $bookingform = new backup_nested_element('bookingform', array('id'), array(
        ...
        $session = new backup_nested_element('session', array('id'), array(
            'bookingform', 'capacity', 'allowoverbook', 'details', 'datetimeknown', 'duration', 'normalcost',
        ...
        // Build the tree.
        $bookingform->add_child($sessions);
        ...
        // Define sources.
        $bookingform->set_source_table('bookingform', array('id' => backup::VAR_ACTIVITYID));

        $session->set_source_table('bookingform_sessions', array('bookingform' => backup::VAR_PARENTID));

        $sessionsdate->set_source_table('bookingform_sessions_dates', array('sessionid' => backup::VAR_PARENTID));

        if ($userinfo) {
            $signup->set_source_table('bookingform_signups', array('sessionid' => backup::VAR_PARENTID));

            $signupstatus->set_source_table('bookingform_signups_status', array('signupid' => backup::VAR_PARENTID));

            $sessionrole->set_source_table('bookingform_session_roles', array('sessionid' => backup::VAR_PARENTID));

            $sessiondataelement->set_source_table('bookingform_session_data', array('sessionid' => backup::VAR_PARENTID));
        }
        ...
        $sessiondataelement->annotate_ids('bookingform_session_field', 'fieldid');

        // Define file annotations.
        // None for F2F.

        // Return the root element (bookingform), wrapped into standard activity structure.
        return $this->prepare_activity_structure($bookingform);
    }
}

 ...
 * @param array $data
 */
function bookingform_send_admin_upgrade_msg($data) {
    global $SITE;
    ...
    $title    = "$SITE->fullname: Booking Form upgrade info";
...
function xmldb_bookingform_upgrade($oldversion=0) {
    global $CFG, $USER, $DB;
    ...
    if ($result && $oldversion < 2008050500) {
        $table = new xmldb_table('bookingform');
    ...
    if ($result && $oldversion < 2008061000) {
        $table = new xmldb_table('bookingform_submissions');
    ...
    if ($result && $oldversion < 2008080100) {
        echo $OUTPUT->notification(get_string('upgradeprocessinggrades', 'bookingform'), 'notifysuccess');
        require_once($CFG->dirroot . '/mod/bookingform/lib.php');
        ...
        $sql = "SELECT f.id, f.name, f.course, s.grade, s.timegraded, s.userid,
            ...
            FROM {bookingform_submissions} s
            JOIN {bookingform} f ON s.bookingform = f.id
        ...
        if ($rs = $DB->get_recordset_sql($sql)) {
            foreach ($rs as $bookingform) {
                $grade = new stdclass();
                $grade->userid = $bookingform->userid;
                $grade->rawgrade = $bookingform->grade;
                $grade->rawgrademin = 0;
                $grade->rawgrademax = 100;
                $grade->timecreated = $bookingform->timegraded;
                $grade->timemodified = $bookingform->timegraded;

                $result = $result && (GRADE_UPDATE_OK == bookingform_grade_item_update($bookingform, $grade));
            }
            $rs->close();
        }
        $DB->debug = true;

        // Remove the grade and timegraded fields from bookingform_submissions.
        if ($result) {
            $table = new xmldb_table('bookingform_submissions');
    ...
    if ($result && $oldversion < 2008090800) {
        ...
        // Define field timemodified to be added to bookingform_submissions.
        $table = new xmldb_table('bookingform_submissions');
    ...
    if ($result && $oldversion < 2009111600) {
        ...
        $table1 = new xmldb_table('bookingform_session_field');
        ...
        $table2 = new xmldb_table('bookingform_session_data');
    ...
    if ($result && $oldversion < 2009111900) {
        ...
        // Remove unused field.
        $table = new xmldb_table('bookingform_sessions');
    ...
    if ($result && $oldversion < 2009112300) {
        ...
        if (!$locationfieldid = $DB->insert_record('bookingform_session_field', $newfield1)) {
            $result = false;
        }
        ...
        if (!$venuefieldid = $DB->insert_record('bookingform_session_field', $newfield2)) {
            $result = false;
        }
        ...
        if (!$roomfieldid = $DB->insert_record('bookingform_session_field', $newfield3)) {
            $result = false;
        }
        ...
        if ($rs = $DB->get_recordset('bookingform_sessions', array(), '', 'id, location, venue, room')) {
    ...
    if ($result && $oldversion < 2009112400) {
        ...
        if ($rs = $DB->get_recordset('bookingform', array(), '', 'id, ' . implode(', ', $templatedfields))) {
            foreach ($rs as $activity) {
                ...
                $result = $result && $DB->update_record('bookingform', $todb);
            }
            $rs->close();
        }
        $DB->debug = $olddebug;
    ...
    if ($result && $oldversion < 2009120900) {
        ...
        try {
            $transaction = $DB->start_delegated_transaction();
            if ($records = $DB->get_records('bookingform_sessions', '', '', '', 'id, bookingform')) {

                // Remove all exising site-wide events (there shouldn't be any).
                foreach ($records as $record) {
                    if (!bookingform_remove_session_from_calendar($record, SITEID)) {
                        $result = false;
                ...
                foreach ($records as $record) {
                    $session = bookingform_get_session($record->id);
                    $bookingform = $DB->get_record('bookingform', 'id', $record->bookingform);

                    if (!bookingform_add_session_to_calendar($session, $bookingform, 'site')) {
                        $result = false;
    ...
    if ($result && $oldversion < 2009122901) {
        ...
        // Create table bookingform_session_roles.
        $table = new xmldb_table('bookingform_session_roles');
        ...
        $table->add_key('sessionid', XMLDB_KEY_FOREIGN, array('sessionid'), 'bookingform_sessions', array('id'));
        $result = $result && $dbman->create_table($table);

        // Create table bookingform_signups.
        $table = new xmldb_table('bookingform_signups');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('sessionid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null);
        $table->add_field('userid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null);
        ...
        // Create table bookingform_signups_status.
        $table = new xmldb_table('bookingform_signups_status');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        ...
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->add_key('signupid', XMLDB_KEY_FOREIGN, array('signupid'), 'bookingform_signups', array('id'));
        $result = $result && $dbman->create_table($table);
        ...
        if ($dbman->table_exists($table)) {
            ...
            $rs = $DB->get_recordset('bookingform_submissions');
            ...
            foreach ($rs as $submission) {
                ...
                $id = $DB->insert_record('bookingform_signups', $signup);

                $signup->id = $id;

                // Check bookingform still exists (some of them are missing).
                // Also, we need the course id so we can load the grade.
                $bookingform = $DB->get_record('bookingform', 'id', $submission->bookingform);
                if (!$bookingform) {

                    // If bookingform delete, ignore as it's of no use to us now.
                    mtrace('Could not find bookingform instance '.$submission->bookingform);
                    continue;
                }

                // Get grade.
                $grade = bookingform_get_grade($submission->userid, $bookingform->course, $bookingform->id);

                // Create initial "booked" signup status.
                $status = new stdClass();
                $status->signupid = $signup->id;
                $status->statuscode = BKF_MDL_F2F_STATUS_BOOKED;
                ...
                $DB->insert_record('bookingform_signups_status', $status);

                // Create attended signup status.
                if ($grade->grade > 0) {
                    $status->statuscode = BKF_MDL_F2F_STATUS_FULLY_ATTENDED;
                ...
                if ($submission->timecancelled) {
                    $status->statuscode = BKF_MDL_F2F_STATUS_USER_CANCELLED;
            ...
            // Drop table bookingform_submissions.
            $table = new xmldb_table('bookingform_submissions');
    ...
    if ($result && $oldversion < 2010051000) {
        ...
        if ($records = $DB->get_records('bookingform_sessions', '', '', '', 'id, bookingform')) {
            ...
            // Remove all exising site-wide events (there shouldn't be any).
            foreach ($records as $record) {
                bookingform_remove_session_from_calendar($record, SITEID);
            }
            ...
            foreach ($records as $record) {
                ...
                bookingform_add_session_to_calendar($session, $bookingform, 'site');
            }
        }
        ...
        $table1 = new xmldb_table('bookingform_notice');
        $table1->add_field('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table1->add_field('name', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        ...
        $table2 = new xmldb_table('bookingform_notice_data');
        $table2->add_field('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table2->add_field('fieldid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, '0');
        $table2->add_field('noticeid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, '0');
        $table2->add_field('data', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        $table2->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table2->add_index('bookingform_notice_date_fieldid', XMLDB_INDEX_NOTUNIQUE, array('fieldid'));
        $result = $result && $dbman->create_table($table2);
    }
    ...
    if ($oldversion < 2011120701) {
        ...
        $badrows = $DB->get_records_sql(
            "
                SELECT
                    *
                FROM
                    {bookingform_session_field}
                WHERE
                    possiblevalues LIKE '%;%'
                AND possiblevalues NOT LIKE '%" . BKF_CUSTOMFIELD_DELIMITER . "%'
                AND type IN (".BKF_CUSTOMFIELD_TYPE_SELECT.",".BKF_CUSTOMFIELD_TYPE_MULTISELECT.")
            "
        );
        ...
        if ($badrows) {
            ...
            foreach ($badrows as $bad) {
                ...
                $fixedrow->id = $bad->id;
                $fixedrow->possiblevalues = str_replace(';', BKF_CUSTOMFIELD_DELIMITER, $bad->possiblevalues);
                $DB->update_record('bookingform_session_field', $fixedrow);
            }
        ...
        $baddatarows = $DB->get_records_sql(
            "
                SELECT
                    sd.id, sd.data
                FROM
                    {bookingform_session_field} sf
                JOIN
                    {bookingform_session_data} sd
                  ON
                    sd.fieldid=sf.id
                WHERE
                    sd.data LIKE '%;%'
                AND sd.data NOT LIKE '%". BKF_CUSTOMFIELD_DELIMITER ."%'
                AND sf.type = ".BKF_CUSTOMFIELD_TYPE_MULTISELECT
        );
        ...
        if ($baddatarows) {
            ...
            foreach ($baddatarows as $bad) {
                $fixedrow = new stdClass();
                $fixedrow->id = $bad->id;
                $fixedrow->data = str_replace(';', BKF_CUSTOMFIELD_DELIMITER, $bad->data);
                $DB->update_record('bookingform_session_data', $fixedrow);
            }

            $transaction->allow_commit();
        }

        upgrade_mod_savepoint(true, 2011120701, 'bookingform');
    }

    if ($oldversion < 2011120702) {
        $table = new xmldb_table('bookingform_session_field');
            ...
            if (!$dbman->index_exists($table, $index)) {
                ...
                if ($rs !== false) {
                    foreach ($rs as $item) {
                        ...
                        $DB->update_record('bookingform_session_field', $data);
                        $replacements[] = array($item['id'], $item['shortname'], $data->shortname);
                    }
                }

                $transaction->allow_commit();
                bookingform_send_admin_upgrade_msg($replacements);
        ...
        upgrade_mod_savepoint(true, 2011120702, 'bookingform');
    }
    ...
    if ($oldversion < 2011120703) {
        ...
        if ($dbman->field_exists($table, $field)) {
            ...
            $bookingforms = $DB->get_records('bookingform');
            foreach ($bookingforms as $bookingform) {
                $bookingform->intro = $bookingform->description;
                $bookingform->introformat = FORMAT_HTML;
                $DB->update_record('bookingform', $bookingform);
            }
        ...
        // Bookingform savepoint reached.
        upgrade_mod_savepoint(true, 2011120703, 'bookingform');
    }
    ...
    if ($oldversion < 2013010400) {
        ...
        // Update the existing showoncalendar field, change true to BKF_F2F_CAL_SITE.
        $sql = 'UPDATE {bookingform}
                SET showoncalendar = ?
                WHERE showoncalendar = ?';
        $DB->execute($sql, array(BKF_F2F_CAL_SITE, BKF_F2F_CAL_COURSE));

        $f2f = $DB->get_records('bookingform');
        foreach ($f2f as $bookingform) {
            bookingform_update_instance($bookingform, false);
        }

        upgrade_mod_savepoint(true, 2013010400, 'bookingform');
    }

    if ($oldversion < 2017053000) {

        // Define field allowcancellationsdefault to be added to bookingform.
        $table = new xmldb_table('bookingform');
        ...
        // Define field allowcancellations to be added to bookingform_sessions.
        $table = new xmldb_table('bookingform_sessions');
        ...
        // Bookingform savepoint reached.
        upgrade_mod_savepoint(true, 2017053000, 'bookingform');
    }

    if ($oldversion < 2019021300) {

        // Define field teacherid to be added to bookingform_sessions.
        $table = new xmldb_table('bookingform_sessions');
        $field = new xmldb_field('teacherid', XMLDB_TYPE_INTEGER, '11', null, null, null, null, 'timemodified');

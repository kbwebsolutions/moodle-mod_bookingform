...
$backtoallsessions = optional_param('backtoallsessions', 0, PARAM_INT); // Bookingform activity to go back to.
...
$potentialuserselector = new bookingform_candidate_selector('addselect', array('sessionid' => $session->id));
$existinguserselector = new bookingform_existing_selector('removeselect', array('sessionid' => $session->id));

// Process incoming user assignments.
if (optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
    require_capability('mod/bookingform:addattendees', $context);
    ...
    if (!empty($userstoassign)) {
        foreach ($userstoassign as $adduser) {
            ...
            if (!has_capability('moodle/course:view', $context, $adduser)) {
                ...
                if (!is_enrolled($context, $user)) {
                    if (!enrol_try_internal_enrol($course->id, $user->id)) {
                        $errors[] = get_string('error:enrolmentfailed', 'bookingform', fullname($user));
                        $errors[] = get_string('error:addattendee', 'bookingform', fullname($user));
            ...
            if (bookingform_get_user_submissions($bookingform->id, $adduser)) {
                $erruser = $DB->get_record('user', array('id' => $adduser), "id, {$usernamefields}");
                $errors[] = get_string('error:addalreadysignedupattendee', 'bookingform', fullname($erruser));
            } else {
                if (!bookingform_session_has_capacity($session, $context)) {
                    $errors[] = get_string('full', 'bookingform');
                ...
                if (!bookingform_user_signup($session, $bookingform, $course, '', BKF_MDL_F2F_BOTH,
                $status, $adduser, !$suppressemail)) {
                    $erruser = $DB->get_record('user', array('id' => $adduser), "id, {$usernamefields}");
                    $errors[] = get_string('error:addattendee', 'bookingform', fullname($erruser));
                }
            }
        }
...
if (optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
    require_capability('mod/bookingform:removeattendees', $context);
    ...
    if (!empty($userstoremove)) {
        foreach ($userstoremove as $removeuser) {
            ...
            if (bookingform_user_cancel($session, $removeuser, true, $cancelerr)) {

                // Notify the user of the cancellation if the session hasn't started yet.
                $timenow = time();
                if (!$suppressemail and !bookingform_has_session_started($session, $timenow)) {
                    bookingform_send_cancellation_notice($bookingform, $session, $removeuser);
                }
            } else {
                $errors[] = $cancelerr;
                $usernamefields = get_all_user_name_fields(true);
                $erruser = $DB->get_record('user', array('id' => $removeuser), "id, {$usernamefields}");
                $errors[] = get_string('error:removeattendee', 'bookingform', fullname($erruser));
            }
        }
        $potentialuserselector->invalidate_selected_users();
        $existinguserselector->invalidate_selected_users();
...
$PAGE->set_cm($cm);
$PAGE->set_url('/mod/bookingform/editattendees.php', array('s' => $s, 'backtoallsessions' => $backtoallsessions));
...
echo $OUTPUT->heading(get_string('addremoveattendees', 'bookingform'));
...
$cells = array();
$content = html_writer::start_tag('p') . html_writer::tag('label', get_string('attendees', 'bookingform'),
...
$content = html_writer::start_tag('p') . html_writer::tag('label',
        get_string('potentialattendees', 'bookingform'), array('for' => 'addselect')) . html_writer::end_tag('p');
$content .= $potentialuserselector->display(true);
$cell = new html_table_cell($content);
$cell->attributes['id'] = 'potentialcell';
$cells[] = $cell;
$table->data[] = new html_table_row($cells);
$content = html_writer::checkbox('suppressemail', 1, $suppressemail, get_string('suppressemail', 'bookingform'),
    array('id' => 'suppressemail'));
$content .= $OUTPUT->help_icon('suppressemail', 'bookingform');
$cell = new html_table_cell($content);
...
$nonattendeesrs = $DB->get_recordset_sql(
     "SELECT
        ...
        AND ss.statuscode = ?
        ORDER BY
            u.lastname, u.firstname", array($session->id, BKF_MDL_F2F_STATUS_REQUESTED)
);
...
foreach ($nonattendeesrs as $user) {
    ...
    $data[] = new html_table_cell(get_string('status_' . bookingform_get_status($user->statuscode), 'bookingform'));
...
if ($nonattendees) {
    ...
    $out .= $OUTPUT->heading(get_string('unapprovedrequests', 'bookingform') . ' (' . $nonattendees . ')');
    $out .= html_writer::table($table);
}
...
$url = new moodle_url('/mod/bookingform/attendees.php', array('s' => $session->id, 'backtoallsessions' => $backtoallsessions));
echo html_writer::link($url, get_string('goback', 'bookingform'));
